...
{
    // Samson Wu
    // Change the code to make EnemyB move back and forth horizontally on its own on the Z axis.

    // Conditions you can modify in editor
    public float distanceToTravel = 5f; // How far (in z-direction) from the starting position you want the object to move
    ...
    void Start()
    {
        ...
        // Calculate the ending position by adding the distance in the z-axis to the starting position
        endPos = new Vector3(transform.position.x, transform.position.y, transform.position.z + distanceToTravel);

        // Start the coroutine that handles the back and forth behaviour in the z-direction
        StartCoroutine(VerticalBehaviourLoop());
    }

    IEnumerator VerticalBehaviourLoop()
    {
